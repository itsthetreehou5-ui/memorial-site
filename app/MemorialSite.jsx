...
export default function MemorialSite() {
  ...
  const fetchThumb = async (clipUrl) => {
    ...
    if (slug) return `https://clips-media-assets2.twitch.tv/${encodeURIComponent(slug)}-preview-480x272.jpg`;
    return null;
  };
  ...
  const addClip = async () => {
    ...
    try {
      ...
      const res = await fetch("/api/clips", {
        ...
        body: JSON.stringify({ title: clipTitle, url: clipUrl, author: clipName || "Anonymous" }),
      });
  ...
  const postMessage = async () => {
    ...
    try {
      ...
      // store delete token
      setTokens(prev => {
        const next = { ...prev, messages: { ...prev.messages, [data.message.id]: data.deleteToken } };
        saveTokens(next); return next;
      });
    } catch(e) {
  ...
  const deleteClip = async (id) => {
    ...
    const res = await fetch(`/api/clips?id=${encodeURIComponent(id)}`, { method:"DELETE", headers:{ "x-delete-token": token } });
    const body = await res.json().catch(()=>({}));
    if (!res.ok) return alert(body?.error || "Could not delete.");
    setClips(p => p.filter(c => c.id !== id));
    setTokens(prev => { const next = { ...prev, clips: { ...prev.clips } }; delete next.clips[id]; saveTokens(next); return next; });
  };
  ...
  const deleteMessage = async (id) => {
    ...
    const res = await fetch(`/api/messages?id=${encodeURIComponent(id)}`, { method:"DELETE", headers:{ "x-delete-token": token } });
    const body = await res.json().catch(()=>({}));
    if (!res.ok) return alert(body?.error || "Could not delete.");
    setMessages(p => p.filter(m => m.id !== id));
    setTokens(prev => { const next = { ...prev, messages: { ...prev.messages } }; delete next.messages[id]; saveTokens(next); return next; });
  };
                  ...
                  (() => {
                    ...
                    return src ? (
                      <div className="embed rounded" style={{ aspectRatio: "16/9" }}>
                        <iframe src={src} allowFullScreen scrolling="no" frameBorder="0" width="100%" height="100%" title={item.title} />
                      </div>
                    ) : (
                      <a className="link-btn" href={item.url} target="_blank" rel="noopener noreferrer">Open on Twitch ↗</a>
                    );
                  })()
                ) : (
                  <div className="embed rounded" style={{ aspectRatio: "16/9" }}>
                    {thumbs[item.id] ? (
                      ...
                      <div style={{ width:"100%", height:"100%", display:"grid", placeItems:"center", color:"#fff" }}>Preview unavailable</div>
                    )}
              ...
              <article className="card" key={`m-${item.id}`}>
                ...
                {tokens.messages[item.id] && (
                  <div style={{ display:"flex", justifyContent:"flex-end", marginTop:8 }}>
                    <button className="btn-tonal btn" onClick={() => deleteMessage(item.id)}>Delete</button>
                  </div>
                )}
              </article>
            )
